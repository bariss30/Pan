...
2. **Önbellek Kurallarını Bypass Etme**: Cloudflare gibi CDN hizmetleri belirli dosya uzantılarını otomatik olarak önbelleğe alır (örneğin, .css, .js, .png). Ancak, bu tür API endpoint’leri genellikle dinamik (DYNAMIC) olarak işaretlendiği için varsayılan olarak önbelleğe alınmaz.
...
Cloudflare’ın yanıt başlığındaki `CF-Cache-Status` değeri, isteğin önbelleğe alınıp alınmadığını gösterir. Normalde dinamik içerikler "DYNAMIC" olarak işaretlenir ve önbelleğe alınmaz. Ancak, `.css` uzantısı eklendiğinde Cloudflare bunu statik bir dosya gibi önbelleğe aldı ve "HIT" olarak döndü.
...
**Saldırgan cache’den auth token’ı alır:**
